...
| 파일 | 행 포함 조건 | 기준 단위 |
| --- | --- | --- |
| `olist_order_item_level.csv` | 전체 주문 상태 | 주문 상품 1건 (`order_id` + `order_item_id`) |
| `olist_delivered_item_level.csv` | `is_delivered == True` 및 `anomaly_flag == 0` | 주문 상품 1건 (`order_id` + `order_item_id`) |

컬럼 유형의 표기 기준은 다음과 같습니다.

| 유형 | 의미 |
| --- | --- |
| `string` | ID, 상태, 도시명, 카테고리 등 문자열 값 |
| `integer` | 순번, 개수, 연월일/시간, 정수 측정값 |
| `float` | 금액, 좌표, 평균 점수 등 소수값 가능 수치 |
...
| 컬럼명 | 유형 | 생성 구분 | 설명 |
| --- | --- | --- | --- |
| `order_id` | string | 원본 | 주문 식별자. 주문 수 계산 시 고유 개수를 사용합니다. |
| `order_item_id` | integer | 원본 | 주문 안에서의 상품 순번. `order_id`와 함께 최종 행의 기준 키입니다. |
| `product_id` | string | 원본 | 상품 식별자입니다. |
| `seller_id` | string | 원본 | 해당 주문 상품을 판매한 판매자 식별자입니다. |
| `shipping_limit_date` | datetime | 원본 | 판매자가 상품 발송을 처리해야 하는 제한 일시입니다. |
| `price` | float | 원본 | 상품 가격입니다. |
| `freight_value` | float | 원본 | 해당 상품 행에 부과된 배송비입니다. |
| `item_total` | float | 파생 | `price + freight_value`로 계산한 상품별 결제 비교용 금액입니다. |
| `customer_id` | string | 원본 | 해당 주문에 연결된 고객 주문 식별자입니다. |
| `order_status` | string | 원본 | 주문 상태입니다. 예: `delivered`, `canceled`, `shipped`. |

### 7.3 주문 시점 및 배송 품질 정보

| 컬럼명 | 유형 | 생성 구분 | 설명 |
| --- | --- | --- | --- |
| `order_purchase_timestamp` | datetime | 원본 | 고객이 주문한 일시입니다. |
| `order_approved_at` | datetime | 원본 | 결제가 승인된 일시입니다. |
| `order_delivered_carrier_date` | datetime | 원본 | 상품이 운송사에 인계된 일시입니다. |
| `order_delivered_customer_date` | datetime | 원본 | 상품이 고객에게 배송된 일시입니다. |
| `order_estimated_delivery_date` | datetime | 원본 | 주문 시 안내된 배송 예정일입니다. |
| `purchase_date` | datetime | 파생 | `order_purchase_timestamp`에서 시간 정보를 제거한 구매 날짜입니다. |
| `purchase_year` | integer | 파생 | 구매 연도입니다. |
| `purchase_month` | integer | 파생 | 구매 월입니다. |
| `purchase_weekday` | string | 파생 | 구매 요일의 영문 이름입니다. 예: `Monday`. |
...
| 컬럼명 | 유형 | 생성 구분 | 설명 |
| --- | --- | --- | --- |
| `customer_unique_id` | string | 원본 | 동일 고객을 식별하기 위한 고유 고객 ID입니다. |
| `customer_zip_code_prefix` | integer | 원본 | 고객 주소의 우편번호 앞자리입니다. |
| `customer_city` | string | 원본 | 고객 도시명입니다. |
| `customer_state` | string | 원본 | 고객 주 코드입니다. 예: `SP`, `RJ`. |
| `customer_state_kor` | string | 파생 | 고객 주 코드의 한글 주명입니다. |
| `customer_region` | string | 파생 | 고객 주를 브라질 5개 권역으로 분류한 값입니다. |
| `customer_geo_lat` | float | 집계 | 고객 우편번호에 해당하는 위치 위도의 평균값입니다. |
| `customer_geo_lng` | float | 집계 | 고객 우편번호에 해당하는 위치 경도의 평균값입니다. |
| `customer_geo_city` | string | 집계 | 고객 우편번호에 해당하는 도시의 최빈값입니다. |
| `customer_geo_state` | string | 집계 | 고객 우편번호에 해당하는 주 코드의 최빈값입니다. |

### 7.5 판매자 지역 정보

| 컬럼명 | 유형 | 생성 구분 | 설명 |
| --- | --- | --- | --- |
| `seller_zip_code_prefix` | integer | 원본 | 판매자 주소의 우편번호 앞자리입니다. |
| `seller_city` | string | 원본 | 판매자 도시명입니다. |
| `seller_state` | string | 원본 | 판매자 원본 주 코드입니다. |
| `seller_state_kor` | string | 파생 | 판매자 주 코드의 한글 주명입니다. |
| `seller_region` | string | 파생 | 판매자 주를 브라질 5개 권역으로 분류한 값입니다. |
| `seller_geo_lat` | float | 집계 | 판매자 우편번호에 해당하는 위치 위도의 평균값입니다. |
| `seller_geo_lng` | float | 집계 | 판매자 우편번호에 해당하는 위치 경도의 평균값입니다. |
| `seller_geo_city` | string | 집계 | 판매자 우편번호에 해당하는 도시의 최빈값입니다. |
| `seller_geo_state` | string | 집계 | 판매자 우편번호에 해당하는 주 코드의 최빈값입니다. |
| `seller_state_geo_mismatch` | boolean | 파생 | `seller_state`와 `seller_geo_state`가 모두 존재하면서 서로 다르면 `True`입니다. |

### 7.6 상품 및 카테고리 정보

| 컬럼명 | 유형 | 생성 구분 | 설명 |
| --- | --- | --- | --- |
| `product_category_name` | string | 원본 | 상품의 포르투갈어 카테고리명입니다. |
| `product_name_lenght` | integer | 원본 | 원본 데이터에 기록된 상품명 길이입니다. 컬럼명의 `lenght` 철자는 원본을 유지합니다. |
| `product_description_lenght` | integer | 원본 | 원본 데이터에 기록된 상품 설명 길이입니다. 컬럼명의 `lenght` 철자는 원본을 유지합니다. |
| `product_photos_qty` | integer | 원본 | 상품 이미지 수입니다. |
| `product_weight_g` | float | 원본 | 상품 무게로 단위는 gram입니다. |
| `product_length_cm` | float | 원본 | 상품 길이로 단위는 centimeter입니다. |
| `product_height_cm` | float | 원본 | 상품 높이로 단위는 centimeter입니다. |
| `product_width_cm` | float | 원본 | 상품 너비로 단위는 centimeter입니다. |
| `product_category_name_english` | string | 원본 결합 | 번역 테이블에서 결합한 상품 카테고리 영문명입니다. 새 계산값은 아닙니다. |

### 7.7 결제 및 리뷰 집계 정보

결제와 리뷰 컬럼은 주문 단위로 집계된 뒤 주문-상품 데이터에 조인됩니다. 따라서 한 주문에 상품이 여러 개이면 같은 집계 값이 여러 행에 반복됩니다.

| 컬럼명 | 유형 | 생성 구분 | 설명 |
| --- | --- | --- | --- |
| `payment_value_total` | float | 집계 | 주문별 `payment_value` 합계입니다. 상품 행 전체를 단순 합산하면 중복 계산됩니다. |
| `payment_installments_max` | integer | 집계 | 주문에 기록된 최대 할부 개월 수입니다. |
| `payment_type_nunique` | integer | 집계 | 주문에서 사용된 고유 결제 수단 수입니다. |
| `payment_types` | string | 집계 | 주문에서 사용된 결제 수단 목록을 쉼표로 결합한 값입니다. |
| `review_score_mean` | float | 집계 | 주문에 연결된 리뷰 점수 평균입니다. |
| `review_count` | integer | 집계 | 주문에 연결된 리뷰 레코드 수입니다. |
| `review_creation_min` | datetime | 집계 | 주문에 연결된 리뷰 중 가장 이른 작성일입니다. |
| `review_answer_max` | datetime | 집계 | 주문에 연결된 리뷰 중 가장 늦은 응답일입니다. |

### 7.8 배송 경로 파생 정보

| 컬럼명 | 유형 | 생성 구분 | 설명 |
| --- | --- | --- | --- |
| `seller_customer_same_state` | boolean | 파생 | 판매자 주와 고객 주가 동일하면 `True`입니다. |
| `delivery_route` | string | 파생 | `seller_state -> customer_state` 형식의 주 단위 배송 경로입니다. 예: `SP -> RJ`. |
